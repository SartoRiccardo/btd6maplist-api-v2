...
class BotAcceptCompletionTest extends TestCase
{
    use TestsBotAuth;
    ...
    public function test_bot_user_without_edit_completion_permission_returns_403(): void
    {
        ...
        // Bot user exists but has no edit:completion permission for MAPLIST
        User::factory()->create(['discord_id' => self::BOT_USER_ID]);

        $this->makeBotSignedRequest()
            ->assertStatus(403)
            ->assertJson(['message' => 'Forbidden - You do not have permission to accept completions for this format.']);
    }
    ...
    #[Group('bot')]
    #[Group('completions')]
    public function test_accept_sets_accepted_by_and_dispatches_job(): void
    {
        $completion = $this->createPendingCompletion(FormatConstants::MAPLIST);

        // Create the bot user with the exact discord_id the trait will use
        ...
        Bus::assertDispatched(UpdateCompletionWebhookJob::class, function ($job) use ($completion) {
            return $job->completionId === $completion->id && $job->fail === false;
        });
    }
}
